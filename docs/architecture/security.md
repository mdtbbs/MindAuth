...
- **CDN header 直读模型**（[request.js](../../src/utils/request.js)）：`app.set('trust proxy', false)`，MindAuth **不依赖** Express 内置的 `req.ip`，也不做受信代理白名单校验。`getClientIp()` 无条件按以下优先级读取 HTTP header：`ali-real-client-ip` → `x-real-ip` → `cf-connecting-ip` → `x-forwarded-for` 首项 → `req.socket.remoteAddress`。每个候选值经 `normalizeIpCandidate` 校验归一化后才使用——**IPv4 与 IPv6 均接受**，自动剥离端口与 IPv6 方括号（`[2001:db8::1]:1234`），`::ffff:` 映射地址还原为 IPv4；非法值跳到下一优先级，全部非法时回退 socket 地址。这使限流、锁定、封禁、审计所用 IP 在所有服务中保持一致。**部署前提**：服务必须部署在会**覆写**这些 header 的 CDN / 反代之后（如 ESA 注入 `ali-real-client-ip`、nginx `proxy_set_header X-Real-IP $remote_addr;`、Cloudflare 注入 `cf-connecting-ip`）。若客户端能直连服务，则可伪造任意 IP——生产部署务必保证直连路径不存在或同样被反代覆盖。
