...
1. **`app.set('trust proxy', false)` 是有意为之**。Express 默认的 `trust proxy` 机制会让任何客户端通过伪造 `X-Forwarded-For` 篡改 `req.ip`，从而绕过按 IP 的限流和封禁。MindAuth 关闭它，IP 提取统一走 `src/utils/request.js` 的 `getClientIp()`——无条件按 `ali-real-client-ip` → `x-real-ip` → `cf-connecting-ip` → `x-forwarded-for` 首项 → socket 地址优先级直读 CDN/反代 header，每个候选经归一化校验。**部署前提**：服务必须部署在会覆写这些 header 的 CDN/反代之后（ESA / nginx / Cloudflare）；若客户端能直连服务则可伪造任意 IP。
