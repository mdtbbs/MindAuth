...
| `request.js` | `getClientIp(req)`, `isValidIpv4(ip)`, `isTrustedProxy(req)`, `normalizeIpCandidate(value)` | 客户端 IP 提取；代理头默认不信任，支持可信代理、Aliyun ESA 自动白名单与 Cloudflare 网段；仅在 trusted-proxy 命中时采信 ESA 注入的 `ali-real-client-ip`，缺失时回退到 `X-Forwarded-For` 首项 |
